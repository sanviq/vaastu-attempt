...
| 7.3 | Added an **About** page (**placeholder — Harshika owns this page and is rewriting it; treat the copy as a draft to replace, not finished work**): what the tool is, a four-step "how it works", the rule set (13 room types, 5 published works, listed by title), who built it, mentor, contact, and a liability disclaimer | Asked for. The source list is read off the same CSV the engine uses, so the page cannot drift from the rules. |
